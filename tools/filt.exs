...
rule =
  rule(
    forall: [
      ...
      has(var(:satellite), :mass, var(:sat_mass), %{when: gte( var(:planet_mass), var(:sat_mass))}),
      has(var(:satellite), :distance, var(:distance)),
      assign(:pull, &(6 * &1[:sat_mass] * &1[:planet_mass] / :math.pow(&1[:distance], 2)))
    ],
    do: [
...
engine =
  ...
  |> assert(:earth, :mass, 55)
  |> assert(:moon, :mass, 7)
  |> assert(:moon, :distance, 380)
