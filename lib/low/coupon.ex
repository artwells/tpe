defmodule Low.Coupon do
  @moduledoc """
  This module defines the `Low.Coupon` schema and provides functions for creating, updating, and retrieving coupons.
  """
  ...
  - `max_count` (`integer`): The maximum count allowed for the coupon.
  ...
  A coupon is considered valid if it is active and the count is not greater than the max_count.
  ...
  - `{:error, :coupon_count_greater_than_max_count}`: If the coupon count is greater than the max_count.
  - `{:error, :coupon_not_found}`: If the coupon is not found.
  """
  ...
  - `promo_id` (`integer`): The ID of the associated promo.
  - `max_chunk` (`integer`): The maximum number of codes to insert in a single transaction.
