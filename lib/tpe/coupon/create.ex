defmodule Tpe.Coupon.Create do
  ...
  def mass_create(count, promo_id, chunk_size, max_use, prefix \\ "", suffix \\ "") do
    ...
    success_count =
      Enum.reduce(chunk(codes, chunk_size), 0, fn chunk, acc ->
      ...
      true ->
        if(Application.fetch_env!(:tpe, :remove_dashes)) do
          Enum.map(1..count, fn _ ->
            %{
              code: String.replace(prefix <> get_single_string() <> suffix, "-", ""),
              promo_id: promo_id,
              max_use: max_use
            }
          end)
  ...
  def insert_coupons_from_csv(file_path) do
    ...
    codes_and_promos =
      ...
      |> CSV.decode()
      |> Enum.map(fn {:ok, [code, promo_id]} ->
        %{code: code, promo_id: String.to_integer(promo_id)}
      end)
      |> Enum.to_list()

    # take codes and promo_ids and insert them in chunks
    success_count =
      Enum.reduce(chunk(codes_and_promos, chunk_size), 0, fn chunk, acc ->
        {:ok, {count, _}} = Tpe.Coupon.Create.insert_coupons(chunk)
        count + acc
      end)
